...
json NFT_Metadata::to_json() const
{
    ...
    {
        attributesJson.push_back(attr.to_json());
    }

    json filesJson = json::array();
    for (const auto &file : files)
    {
        filesJson.push_back(file.to_json());
    }

    json creatorsJson = json::array();
    for (const auto &creator : creators)
    {
        creatorsJson.push_back(creator.to_json());
    }
...
{
    if (j.contains("name") && j.contains("symbol") && j.contains("description") && j.contains("seller_fee_basis_points") && j.contains("image") && j.contains("external_url") &&
